...
def generate_collage(image_paths, date_str, title):
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        # Taille aléatoire : chaque photo peut être plus grande ou plus petite (70% à 110%)
        # size_factor = random.uniform(0.7, 1.1)
        # annulation du random de taille
        size_factor = 1.3
        ...
        # Calculer le centre de la cellule avec BEAUCOUP d'aléa (désalignement)
        # Variation horizontale et verticale jusqu'à 40% de la cellule
        alea = 0.15
    ...
    try:
        ...
        print(f"✅ Collage généré : {output} avec EXIF ({dt_str})")
    except Exception as e:
        print(f"⚠ Impossible de modifier les EXIF : {e}")

if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python script.py <date:YYYY-MM-DD> <titre> <img1> [<img2> ... <img7>]")
        sys.exit(1)
    ...
    generate_collage(image_paths, date_str, title)